...
class TestColumn:
    def test_type_casting(self) -> None:
        data = [1, 2, 3]
        column = Column(data)
        assert_that(column.values.tolist(), equal_to([1, 2, 3]))
        ...
        assert_that(column.values.tolist(), equal_to([1.23, 2.23, 3.23]))

        column = Column[str]([1, 2, 3])
        assert_that(column.values.tolist(), equal_to(["1", "2", "3"]))

        column = Column[bool]([0, 1, 2])
        assert_that(column.values.tolist(), equal_to([False, True, True]))

        column: Column[bool] = Column([0, 1, 2])
        assert_that(column.values.tolist(), equal_to([0, 1, 2]))

        try:
            Column[float](["a", "b", "c"])
            fail("Expected cast exception")
        except Exception as e:
            assert_that("Failed to cast 'str' to 'float'", is_in(str(e)))
    ...
    def test_multi_threaded(self) -> None:
        ...
        for _ in range(1000):
            column = Column(data)
            assert_that(column.values.tolist(), equal_to(data))
